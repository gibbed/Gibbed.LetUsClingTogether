...
{
    public class FloatDescriptor : IDescriptor
    {
        private readonly PrimitiveType _Type;
        private readonly int _MinimumWidth;

        public FloatDescriptor(PrimitiveType type, int minimumWidth)
        {
            if (minimumWidth < 0)
            {
            ...
            if (type.IsFloat() == false)
            {
                throw new ArgumentException("not a float type", nameof(type));
            }
